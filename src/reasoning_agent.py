...
def generate_event_report(event: ConjunctionEvent, kb: KnowledgeBase) -> dict:
    ...
    prompt = (
        ...
        "operational, not generic. Note that the probability_of_collision figure "
        "uses an assumed, generic covariance (not real tracking-derived "
        "covariance), so treat it as illustrative rather than authoritative, and "
        "anchor your assessment primarily on miss distance and risk tier.\n\n"
